      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a0f1e,#141c2e)", padding: 24, paddingTop: 80, color: "white" }}>
            ...
            {watchlist.map(item => (
              <div key={item.id}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    onClick={e => { e.stopPropagation(); setAlertCoin(item); setShowAlertModal(true); }}
                    style={{ padding: "6px 14px", borderRadius: 999, border: "1px solid #3b82f6", background: "rgba(59,130,246,.2)", color: "#3b82f6" }}>
                    Add Alert
                  </button>
        ...
        {showAlertModal && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ width: 420 }}>
              <AlertPanel coin={alertCoin} onSaved={() => { setShowAlertModal(false); loadWatchlist(); }} onClose={() => setShowAlertModal(false)} />
            </div>
          </div>
        )}
