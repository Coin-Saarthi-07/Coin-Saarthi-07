...
//        plan.setMaxWatchlistItems(dto.getMaxWatchlistItems());
	...
	public List<SubscriptionPlanDTO> getAllPlans() {
		return planRepo.findAll().stream().map(plan -> new SubscriptionPlanDTO(plan.getPlanId(),
				plan.getPlanName(),plan.getFeatures(), plan.getPlanPrice(), plan.getDuration())).toList();
	}
